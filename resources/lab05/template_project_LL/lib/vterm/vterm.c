...
/**
 * @brief Инициализация USART3, подключённого к STLinkv3 CDC
 * @note  PD8(AF7)->USART3_TX, PD9(AF7)->USART3_RX; kernel clock = HSI
...
static void uart_init(int baudrate) {
  ...
  MODIFY_REG(RCC->D2CCIP2R, RCC_D2CCIP2R_USART28SEL_Msk,
  ...
  // Назначаем номер альт. функции PD8 и PD9, соотв. USART3
  ...
  // Разрешаем передатчик и приёмник; остальные параметры кадра (8 бит данных,
  // без контроля чётности, 1 стоповый бит, oversampling 16) - по умолчанию
  MODIFY_REG(USART3->CR1, USART_CR1_TE | USART_CR1_RE, USART_CR1_TE | USART_CR1_RE);
  // Предделитель kernel clock: HSI/8 (8 МГц)
  ...
  // Отключаем регистрацию ошибки переполнения приёмника (overrun)
...
/**
 * @brief Передача одного символа в USART3  */
static inline void uart_putch(char ch) {
  // ожидание готовности регистра передатчика принять следующий символ
  BLOCK_ON((USART3->ISR & USART_ISR_TXE_TXFNF) == 0);
  USART3->TDR = ch;
}
/**
...
int _read(int file, char *ptr, int len) {
  if (lib_init_flag && len > 0 && file == 0) {
    ...
    while (i < len) {
      // Короткое ожидание следующего символа
