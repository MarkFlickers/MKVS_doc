...
Темы, которые проходят через курс: язык C, технологии разработки ПО, библиотеки, загрузка, прерывания, таймеры, [[glossary/gpio\|GPIO]], [[glossary/usart\|UART]], [[glossary/rtos\|RTOS]].
...
| 03 | [[lab03/index\|Исключения и прерывания. Обработка ошибок. Загрузчик]] | перенесена из PDF (r1) |
| 04 | [[lab04/index\|Системный таймер. Программные таймеры. Кооперативный планировщик. Библиотека LL]] | перенесена из PDF (r2) |
| 05 | [[lab05/index\|Контроллер внешних прерываний EXTI. Поворотный инкрементальный энкодер]] | перенесена из PDF (r0) |
| 06 | [[lab06/index\|Библиотека HAL. Последовательный асинхронный интерфейс UART]] | перенесена из PDF (r2) |
| 07 | [[lab07/index\|Аппаратные таймеры]] | перенесена из PDF (r1) |
| 08 | [[lab08/index\|Основы встраиваемых операционных систем реального времени на примере FreeRTOS]] | перенесена из PDF (r1) |

> [!note] О переносе в Markdown
> Тексты пособий перенесены из исходных файлов Word/PDF.
> В ЛР1 смысловых правок не вносилось. В ЛР2 выправлены опечатки, грамматика и пунктуация
> исходного текста, а в ЛР3—ЛР8 — дополнительно неточности формулировок и плохо
> построенные фразы. В ЛР4—ЛР8 выправлены и листинги: устранены ошибки и места,
> вызывавшие предупреждения компилятора.
...
**Программный код** оформляется двумя способами.

Код, который следует **перепечатать вручную**: пунктирная рамка, пиктограмма клавиатуры в углу, кнопки копирования нет, текст нельзя выделить мышью.
...
Код, который можно **скопировать через буфер обмена**: сплошная рамка и кнопка копирования в правом верхнем углу (CTRL+C — копировать, CTRL+V — вставить). После вставки нажмите ALT+SHIFT+F в VS Code для восстановления форматирования.
