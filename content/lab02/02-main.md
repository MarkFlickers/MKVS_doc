...
1. Откройте и изучите [[glossary/linker-script\|скрипт компоновщика]] `h745cm7_flash.ld`.
   ...
   ![[glossary/linker#^def-linker]]

   ![[glossary/elf#^def-elf]]
   ...
   ![[glossary/sections#^def-sections]]
...
> 2.2. Найдите определение функции `Reset_Handler` в секции `.text`. Попытайтесь составить обобщённый алгоритм функции `Reset_Handler` и запишите его в конспект.

   2.3. Найдите в коде реализацию обработчика «по умолчанию» `Default_Handler`.
...
> - `cmp` и `bcc` — сравнение и условный переход, если результат сравнения «меньше» (для чисел без знака). Переходу «больше или равно» соответствует `bcs`.
...
3. Откройте файл `system/system.c` и изучите код системного файла.
   ...
   3.3. Согласно стандарту [[glossary/cmsis\|CMSIS]] определяются глобальная переменная [[glossary/system-core-clock\|SystemCoreClock]] для хранения значения частоты системного тактового сигнала и функция `SystemCoreClockUpdate()` для вычисления (обновления) значения этой переменной по состоянию регистров МК.

4. Откройте файл `src/hello_led.c` и изучите код программы.

   Обратите внимание на то, что функция `main()` содержит цикл, который никогда не завершается ([[glossary/superloop\|суперцикл]]).

   ![[glossary/superloop#^def-superloop]]

   Светодиоды отладочной платы подключены к выводам через токоограничивающие резисторы. Например, жёлтый светодиод LD2 подключён к выводу PE1 (рисунок 6).

![Схема подключения жёлтого светодиода LD2 к выводу PE1](img/fig-06-led2-schematic.png)

*Рисунок 6 – Схема подключения жёлтого светодиода LD2 (USER) к выводу PE1.*
   ...
   ![[glossary/push-pull#^def-push-pull]]

   Прежде чем работать с регистрами периферийного блока, необходимо включить тактирование этого блока.

   ![[glossary/rcc#^def-rcc]]

   Функция `myled_enable()` включает тактирование порта GPIOE.
...
2. Перед использованием функций библиотеки [[glossary/vterm\|vterm]] или функций ввода-вывода стандартной библиотеки необходимо подключить заголовочный файл (`#include <vterm.h>`) и вызвать функцию `vterm_init()`, указав скорость передачи данных, например 115 200 бит/с.

3. Добавьте библиотеку `vterm` в проект (см. [[lab02/index#Файлы к работе\|файлы работы]]). Библиотека состоит из файлов `vterm.h` и `vterm.c`, которые следует разместить в папке `lib/vterm`. Листинги файлов можно найти в приложении или в архиве файлов работы.
...
2. Проанализируйте код конфигурационных файлов.
   ...
   2.3. Код функции задержки `delay()` отличается от аналогичной функции в программе `hello_led.c`.

   ![[glossary/dwt#^def-dwt]]

   Регистр `CYCCNT` этого блока содержит счётчик циклов, выполненных процессором. Использование этого счётчика позволяет не делать предположений о количестве операций, затрачиваемых циклом проверки.
