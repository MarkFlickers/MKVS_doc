...
3. Откройте файл `system/system.c` и изучите код системного файла.
   ...
   3.3. Согласно стандарту [[glossary/cmsis\|CMSIS]] определяются глобальная переменная [`SystemCoreClock`](../glossary/system-core-clock) для хранения значения частоты системного тактового сигнала и функция `SystemCoreClockUpdate()` для вычисления (обновления) значения этой переменной по состоянию регистров МК.
