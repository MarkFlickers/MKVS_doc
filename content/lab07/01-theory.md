...
7.2. Существуют два режима захвата (рисунок 7). В режиме X2 счётчик изменяется по каждому фронту только одного канала — `T1` или `T2`. В режиме X4 счётчик обновляется по каждому фронту обоих каналов, то есть даёт вчетверо больше отсчётов на один оборот.
