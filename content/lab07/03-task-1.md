...
1. Выполните подключение датчика HC-SR04 согласно варианту.
   ...
   **Вариант 1** — подключение к разъёму CN10 отладочной платы: сигнал `Trig` на вывод PB0 (контакт 31), сигнал `Echo` на вывод PA0 (контакт 29). Питание и земля датчика подключаются к контактам 33 и 27 того же ряда, как показано на рисунке 12.
...
> Запускающий импульс `Trig` длится не менее 10 мкс, а ответный импульс `Echo` — от 100 мкс до 36 мс. Подберите предделители таймеров так, чтобы оба интервала укладывались в разрядность счётчиков с приемлемой точностью: частота счёта 1 кГц, взятая в основной части работы, для микросекундных интервалов уже не годится.
