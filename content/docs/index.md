...
Техническая документация, с которой ведётся работа в лабораторных работах: reference manuals, даташиты, программные модели ядер, руководства на отладочную плату и модули расширения.
...
> [!tip] Ссылки ведут на конкретную страницу
> Рядом с каждым документом собраны быстрые переходы вида «12 GPIO». Такая ссылка открывает PDF сразу на нужной странице, а не в начале. Номер страницы совпадает с напечатанным в колонтитуле документа.
>
> Документы сохранены в режиме Fast Web View (линеаризованы), поэтому браузер показывает запрошенную страницу, не дожидаясь загрузки файла целиком. Для RM0399 объёмом 67 МБ это заметно: раздел открывается за секунды. Так умеют встроенные просмотрщики Chrome, Edge и Firefox; сторонние программы могут скачивать файл полностью.
